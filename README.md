...
Options:
  ...
  * `--algo`: algorithm used to select the key frames of the video.
    ...
    * `fid` : medoids retrieved with k-medoids clustering on InceptionV3 latent space vectors corresponding to each video frame. The metric used to compute the distance between two latent vectors is the [Frechet Inception Distance (FID)](https://en.wikipedia.org/wiki/Fr%C3%A9chet_inception_distance).
